...
int main()
{
	...
	//setup
	Player player; player.setPosition({ 0, 0, 80});
	bool canJump;
	...
	while (running)
	{
		...
		while (window.pollEvent(event))
		{
			...
			else if (event.type == sf::Event::Resized)
			{
				...
				if (event.mouseButton.button == sf::Mouse::Button::Left) {
					...
					std::cout << ray.blockPos.x << "--" << ray.blockPos.z << "--" << ray.blockPos.y << "-----------" << ray.side << "\n";
					if (ray.isCollision)
					...
					RayCollision ray = world->raycast(camera.getPosition(), camera.getDirectionVector(), 4);
					std::cout << ray.blockPos.x << "--" << ray.blockPos.z << "--" << ray.blockPos.y << "-----------" << ray.side << "-----------" << world->getSunlightLevel(ray.blockPos) << "\n";
					if (ray.isCollision && !player.checkCollision(floor(ray.blockPos + adjecent[ray.side])))
		...
		if (cameraControl) {
			camera.rotate(float(1024 / 2 - sf::Mouse::getPosition().x), float(768 / 2 - (sf::Mouse::getPosition().y - 2100)));
