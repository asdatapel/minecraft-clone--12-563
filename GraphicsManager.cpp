...
GraphicsManager::GraphicsManager()
{
	...
	glTexStorage3D(GL_TEXTURE_2D_ARRAY, 1, GL_RGBA, textureWidth, textureHeight, textureCount);
...
int GraphicsManager::newShaderResource(std::string vertShader, std::string fragShader, std::list<Attribute> attributes)
{
...
int GraphicsManager::getArrayTexture(std::string name)
{
	...
	glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, next, tex.image.getSize().x, tex.image.getSize().y, 1, GL_RGBA, GL_UNSIGNED_BYTE, tex.image.getPixelsPtr());
...
int GraphicsManager::newBuffer(int shaderId)
{
	...
	for (auto const& attrib : shaderResources[shaderId].attributes) {
		...
		glVertexAttribPointer(attrib.handle, attrib.length, GL_FLOAT, GL_FALSE, attrib.stride * sizeof(GLfloat), (void*) (attrib.distanceToFirst * sizeof(float)));
	}
